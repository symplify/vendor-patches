...
use VendorPatches202511\Symfony\Component\Console\Helper\HelperInterface;
...
class Command implements SignalableCommandInterface
{
    ...
    public static function getDefaultName() : ?string
    {
        ...
        if ($attribute = \method_exists(new \ReflectionClass(static::class), 'getAttributes') ? (new \ReflectionClass(static::class))->getAttributes(AsCommand::class) : []) {
    ...
    public function __construct(?string $name = null)
    {
        ...
        $attribute = ($nullsafeVariable1 = (\method_exists(new \ReflectionClass(static::class), 'getAttributes') ? (new \ReflectionClass(static::class))->getAttributes(AsCommand::class) : [])[0] ?? null) ? $nullsafeVariable1->newInstance() : null;
        if (null === $name) {
        ...
        if ('' === $this->description) {
            if (self::class !== (new \ReflectionMethod($this, 'getDefaultDescription'))->class) {
                ...
                $defaultDescription = ($nullsafeVariable3 = $attribute) ? $nullsafeVariable3->description : null;
            }
            $this->setDescription($defaultDescription ?? '');
        }
        if ('' === $this->help) {
            $this->setHelp((($nullsafeVariable4 = $attribute) ? $nullsafeVariable4->help : null) ?? '');
        }
    ...
    public function addArgument(string $name, ?int $mode = null, string $description = '', $default = null, $suggestedValues = [])
    {
        ...
        ($nullsafeVariable5 = $this->fullDefinition) ? $nullsafeVariable5->addArgument(new InputArgument($name, $mode, $description, $default, $suggestedValues)) : null;
        return $this;
    }
    /**
     * Adds an option.
    ...
    public function addOption(string $name, $shortcut = null, ?int $mode = null, string $description = '', $default = null, $suggestedValues = [])
    {
        ...
        ($nullsafeVariable6 = $this->fullDefinition) ? $nullsafeVariable6->addOption(new InputOption($name, $shortcut, $mode, $description, $default, $suggestedValues)) : null;
        return $this;
    }
    /**
    ...
    {
        $name = $this->name;
        $isSingleCommand = ($nullsafeVariable7 = $this->application) ? $nullsafeVariable7->isSingleCommand() : null;
    ...
    {
        return (($nullsafeVariable8 = $this->code) ? $nullsafeVariable8->getSubscribedSignals() : null) ?? [];
    }
    /**
    ...
    {
        return (($nullsafeVariable9 = $this->code) ? $nullsafeVariable9->handleSignal($signal, $previousExitCode) : null) ?? \false;
    }
    /**
