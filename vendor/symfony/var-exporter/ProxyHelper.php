 ...
 */
namespace VendorPatches202212\Symfony\Component\VarExporter;

use VendorPatches202212\Symfony\Component\VarExporter\Exception\LogicException;
use VendorPatches202212\Symfony\Component\VarExporter\Internal\Hydrator;
use VendorPatches202212\Symfony\Component\VarExporter\Internal\LazyObjectRegistry;
/**
...
EOPHP;
    ...
    public static function exportSignature(\ReflectionFunctionAbstract $function, bool $withParameterTypes = \true) : string
    {
        ...
        foreach ($function->getParameters() as $param) {
            $parameters[] = ((\method_exists($param, 'getAttributes') ? $param->getAttributes(\VendorPatches202212\SensitiveParameter::class) : []) ? '#[\\SensitiveParameter] ' : '') . ($withParameterTypes && $param->hasType() ? self::exportType($param) . ' ' : '') . ($param->isPassedByReference() ? '&' : '') . ($param->isVariadic() ? '...' : '') . '$' . $param->name . ($param->isOptional() && !$param->isVariadic() ? ' = ' . self::exportDefault($param) : '');
        }
