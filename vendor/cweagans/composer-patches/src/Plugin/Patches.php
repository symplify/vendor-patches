 ...
 */
namespace VendorPatches202605\cweagans\Composer\Plugin;

use VendorPatches202605\Composer\Composer;
use VendorPatches202605\Composer\DependencyResolver\Operation\InstallOperation;
use VendorPatches202605\Composer\DependencyResolver\Operation\OperationInterface;
use VendorPatches202605\Composer\DependencyResolver\Operation\UpdateOperation;
use VendorPatches202605\Composer\EventDispatcher\EventDispatcher;
use VendorPatches202605\Composer\EventDispatcher\EventSubscriberInterface;
use VendorPatches202605\Composer\Installer\PackageEvent;
use VendorPatches202605\Composer\Installer\PackageEvents;
use VendorPatches202605\Composer\IO\IOInterface;
use VendorPatches202605\Composer\Json\JsonFile;
use VendorPatches202605\Composer\Package\PackageInterface;
use VendorPatches202605\Composer\Plugin\Capability\CommandProvider as CommandProviderCapability;
use VendorPatches202605\Composer\Plugin\Capable;
use VendorPatches202605\Composer\Plugin\PluginInterface;
use VendorPatches202605\Composer\Util\ProcessExecutor;
use VendorPatches202605\cweagans\Composer\Capability\CommandProvider;
use VendorPatches202605\cweagans\Composer\Capability\Downloader\CoreDownloaderProvider;
use VendorPatches202605\cweagans\Composer\Capability\Downloader\DownloaderProvider;
use VendorPatches202605\cweagans\Composer\Capability\Patcher\CorePatcherProvider;
use VendorPatches202605\cweagans\Composer\Capability\Patcher\PatcherProvider;
use VendorPatches202605\cweagans\Composer\Capability\Resolver\CoreResolverProvider;
use VendorPatches202605\cweagans\Composer\Capability\Resolver\ResolverProvider;
use VendorPatches202605\cweagans\Composer\ConfigurablePlugin;
use VendorPatches202605\cweagans\Composer\Downloader;
use VendorPatches202605\cweagans\Composer\Event\PatchEvent;
use VendorPatches202605\cweagans\Composer\Event\PatchEvents;
use VendorPatches202605\cweagans\Composer\Locker;
use VendorPatches202605\cweagans\Composer\Patch;
use VendorPatches202605\cweagans\Composer\PatchCollection;
use VendorPatches202605\cweagans\Composer\Patcher;
use VendorPatches202605\cweagans\Composer\Resolver;
use VendorPatches202605\cweagans\Composer\Util;
...
class Patches implements PluginInterface, EventSubscriberInterface, Capable
{
    ...
    {
        $composer_file = \VendorPatches202605\Composer\Factory::getComposerFile();
    ...
    public function activate(Composer $composer, IOInterface $io) : void
    {
        ...
        $this->lockFile = new JsonFile(static::getPatchesLockFilePath(), null, $this->io);
        $this->locker = new Locker($this->lockFile);
        $this->configuration = ['disable-resolvers' => ['type' => 'list', 'default' => []], 'disable-downloaders' => ['type' => 'list', 'default' => []], 'disable-patchers' => ['type' => 'list', 'default' => []], 'default-patch-depth' => ['type' => 'int', 'default' => 1], 'package-depths' => ['type' => 'list', 'default' => []], 'patches-file' => ['type' => 'string', 'default' => 'patches.json'], "ignore-dependency-patches" => ['type' => 'list', 'default' => []]];
    ...
    {
        return [ResolverProvider::class => CoreResolverProvider::class, DownloaderProvider::class => CoreDownloaderProvider::class, PatcherProvider::class => CorePatcherProvider::class, CommandProviderCapability::class => CommandProvider::class];
    }
    /**
    ...
    public function download(Patch $patch)
    {
        ...
        $this->composer->getEventDispatcher()->dispatch(PatchEvents::PRE_PATCH_DOWNLOAD, new PatchEvent(PatchEvents::PRE_PATCH_DOWNLOAD, $patch, $this->composer, $this->io));
        $downloader->downloadPatch($patch);
        $this->composer->getEventDispatcher()->dispatch(PatchEvents::POST_PATCH_DOWNLOAD, new PatchEvent(PatchEvents::POST_PATCH_DOWNLOAD, $patch, $this->composer, $this->io));
    }
    public function guessDepth(Patch $patch)
    {
        ...
        $depth = $patch->depth ?? $this->getConfig('package-depths')[$patch->package] ?? Util::getDefaultPackagePatchDepth($patch->package) ?? $this->getConfig('default-patch-depth');
        $patch->depth = $depth;
    }
    public function apply(Patch $patch, string $install_path)
    {
        ...
        $patch = $event->getPatch();
        $this->io->write("      - Applying patch <info>{$patch->localPath}</info> (depth: {$patch->depth})", \true, IOInterface::DEBUG);
        ...
        if ($status === \false) {
            ...
            $this->composer->getEventDispatcher()->dispatch(PatchEvents::POST_PATCH_APPLY_ERROR, new PatchEvent(PatchEvents::POST_PATCH_APPLY_ERROR, $patch, $this->composer, $this->io, $e));
            throw $e;
        }
        $this->composer->getEventDispatcher()->dispatch(PatchEvents::POST_PATCH_APPLY, new PatchEvent(PatchEvents::POST_PATCH_APPLY, $patch, $this->composer, $this->io));
    }
    /**
    ...
    public function patchPackage(PackageEvent $event)
    {
        ...
        if (empty($this->patchCollection->getPatchesForPackage($package->getName()))) {
            $this->io->write("No patches found for <info>{$package->getName()}</info>", \true, IOInterface::DEBUG);
            return;
        }
        $install_path = $this->composer->getInstallationManager()->getInstaller($package->getType())->getInstallPath($package);
        ...
        foreach ($this->patchCollection->getPatchesForPackage($package->getName()) as $patch) {
            /** @var $patch Patch */
            // Download patch.
            $this->io->write("    - Downloading and applying patch <info>{$patch->url}</info> ({$patch->description})", \true, IOInterface::VERBOSE);
            $this->io->write("      - Downloading patch <info>{$patch->url}</info>", \true, IOInterface::DEBUG);
            $this->download($patch);
            $this->guessDepth($patch);
            // Apply patch.
            $this->io->write("      - Applying downloaded patch <info>{$patch->localPath}</info>", \true, IOInterface::DEBUG);
            $this->apply($patch, $install_path);
        }
        $this->io->write("  - All patches for <info>{$package->getName()}</info> have been applied.", \true, IOInterface::DEBUG);
    }
    /**
