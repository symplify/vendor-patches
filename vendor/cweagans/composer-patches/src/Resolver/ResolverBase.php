 ...
 */
namespace VendorPatches202605\cweagans\Composer\Resolver;
...
abstract class ResolverBase implements ResolverInterface
{
     ...
     * {@inheritdoc}
     */
    public abstract function resolve(PatchCollection $collection) : void;
    /**
    ...
    public function findPatchesInJson(array $patches) : array
    {
        ...
        foreach ($patches as $package => $patch_defs) {
            if (isset($patch_defs[0]) && \is_array($patch_defs[0])) {
                $this->io->write("    Using expanded definition format for package <info>{$package}</info>", \true, IOInterface::VERBOSE);
                ...
                $this->io->write("    Using compact definition format for package <info>{$package}</info>", \true, IOInterface::VERBOSE);
