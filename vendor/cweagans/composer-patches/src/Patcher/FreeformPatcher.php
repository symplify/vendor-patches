...
class FreeformPatcher extends PatcherBase
{
    public function apply(Patch $patch, string $path) : bool
    {
        ...
        if (empty($patchTool) || empty($args)) {
            $this->io->write('Required configuration for FreeformPatcher not present. Skipping.', \true, IOInterface::VERBOSE);
            return \false;
        }
        // If we have dry-run args, do a dry-run.
        if (!empty($dryRunArgs)) {
            $status = $this->executeCommand('%s ' . $dryRunArgs, $patchTool, $patch->depth, $path, $patch->localPath);
            if (!$status) {
                return \false;
            }
        }
        // Apply the patch.
        return $this->executeCommand('%s ' . $args, $patchTool, $patch->depth, $path, $patch->localPath);
    }
