...
namespace VendorPatches202605\cweagans\Composer;

use VendorPatches202605\Composer\Composer;
use VendorPatches202605\cweagans\Composer\Downloader\DownloaderInterface;
...
use VendorPatches202605\cweagans\Composer\Event\PluginEvent;
use VendorPatches202605\cweagans\Composer\Event\PluginEvents;
...
class Downloader
{
    ...
    public function downloadPatch(Patch $patch)
    {
        ...
        foreach ($this->getDownloaders() as $downloader) {
            ...
            if (\in_array($class, $this->disabledDownloaders, \true)) {
                $this->io->write('<info>  - Skipping downloader ' . $class . '</info>', \true, IOInterface::VERBOSE);
                continue;
            }
    ...
    protected function getDownloaders() : array
    {
        ...
        $capabilities = $plugin_manager->getPluginCapabilities(DownloaderProvider::class, ['composer' => $this->composer, 'io' => $this->io]);
