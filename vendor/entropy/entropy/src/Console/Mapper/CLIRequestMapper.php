...
use VendorPatches202602\Entropy\Console\Exception\ConsoleInputMappingException;
use VendorPatches202602\Entropy\Console\ValueObject\CLIRequest;
use VendorPatches202602\Entropy\Tests\Console\Mapper\CLIRequestMapperTest;
    ...
    public function resolveArguments(CommandInterface $command, CLIRequest $cliRequest) : array
    {
        ...
        foreach ($reflectionMethod->getParameters() as $key => $reflectionParameter) {
            ...
            if ($key !== 0 && (string) $reflectionParameter->getType() === 'array') {
                if (\substr_compare($optionName, 's', -\strlen('s')) === 0) {
