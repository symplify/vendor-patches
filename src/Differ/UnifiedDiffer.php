...
final class UnifiedDiffer
{
    ...
    private const int ADDED = 1;

    private const int REMOVED = 2;

    private const int NO_LINE_END_EOF_WARNING = 4;
    ...
    private function diffToArray(array $from, array $to): array
    {
        ...
        while ($suffix < $minLength - $prefix
            && $from[$fromLength - 1 - $suffix] === $to[$toLength - 1 - $suffix]) {
            ++$suffix;
        }
    ...
    private function markMissingEndOfFileNewline(array $diff): array
    {
        ...
        if ($diff[$lastIndex][1] === self::OLD) {
            if (! str_ends_with($diff[$lastIndex][0], "\n")) {
